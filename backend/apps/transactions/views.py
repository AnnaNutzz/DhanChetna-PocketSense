...
class TransactionViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        # ML classification (Phase 1: rules-based)
        if transaction.type == 'expense':
            from .classifier import classify_expense
            auto_cat, confidence = classify_expense(transaction)
            transaction.auto_category = auto_cat
            transaction.category_confidence = confidence
            transaction.save(update_fields=['auto_category', 'category_confidence'])
